...
test('HCI 与 DMP 切换只改变授权场景显隐，不改变归属列宽状态', () => {
  const { dom, document, window, errors } = loadV2Dom();
  try {
    const product = document.getElementById('plname');
    const sceneItem = document.getElementById('authSceneFormItem');
    ...
    assert.equal(affiliationRow.classList.contains('has-auth-scene'), false, 'HCI 不应附加动态列宽状态');

    product.value = '24';
    fireChange(product, window);
    assert.equal(sceneItem.hidden, true, 'DMP 未识别设备时应隐藏授权场景');
    assert.equal(affiliationRow.classList.contains('has-auth-scene'), false, 'DMP 不应通过状态类拉宽归属字段');
    assert.deepEqual(errors, []);
  } finally {
    dom.window.close();
  }
